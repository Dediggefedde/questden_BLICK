declare function postpreview(arg1: string, arg2: string, arg3: string, arg4: string): void;
declare function addtowatchedthreads(id: string, board: string): void;
declare function getwatchedthreads(id: string, board: string): void;
declare const JSZip: { new(): JSZip; prototype: JSZip; };
interface JSZip {
	file(name: string, data: string | ArrayBuffer | Blob, options?: { compression?: "STORE" | "DEFLATE", binary?: boolean }): JSZip;
	generateAsync(
...
(function () {
	...
	//
	function rgb2hsl(r: number, g: number, b: number): [hue: number, sat: number, lum: number] {
		r /= 255; g /= 255; b /= 255;
	...
	abstract class Storage<T extends Record<string, any>> {
		...
		read(str: string | null) {
			...
			try {
				const parsedData: Partial<T> = this.parse(str);
				this.data = { ...this.default };
				for (const key of Object.keys(parsedData) as (keyof T)[]) {
					this.data[key] = parsedData[key] as T[keyof T];
				}
			} catch (ex) {
		...
		set<K extends keyof T>(key: K, value: T[K] | undefined = this.default[key]): void {
			if (key in this.data) {
	...
	class Sync {
		...
		async upload<T extends Record<string, any>>(name: string, storage: Storage<T> | null): Promise<void> {
		...
		async download<T extends Record<string, any>>(name: string, storage: Storage<T> | null): Promise<void> {
			if (storage === null) return;
			if (!this.creds.data.token) {
				throw new Error('Not authenticated');
			}
	...
	//GUI interaction
	class Sidebar {
		private tabSet!: HTMLDivElement;
		private tabStyle!: HTMLStyleElement;
		private dom: DOMManip;
		private fexp: FileExport;
	...
	`;
		constructor(dom: DOMManip, set: Settings, syncr: Sync) {
			this.dom = dom;
			this.syncr = syncr;
			this.setting = set;
			this.addElements();
		...
		//
		loadSettings() { //sets the interface to the given settings
			let el: HTMLInputElement | null;
			for (const key in this.setting.data) {
				el = document.querySelector("#BLICK_" + key);
				if (el === null) continue;
				if (typeof (this.setting.data[key]) === "boolean") el.checked = this.setting.data[key] as boolean;
				if (typeof (this.setting.data[key]) === "number") el.value = String(this.setting.data[key]);
			}
			el = document.querySelector(`#BLICK_fonttype option[value='${this.setting.data.fonttype}']`);
			if (el !== null) el.setAttribute("selected", "selected");
		...
		//
		fillFontList() { //adds font entries from `fontTypeList` to setting dropbox
			const cont: HTMLSelectElement | null = document.querySelector("#BLICK_fonttype");
			if (cont === null) return console.log("BLICK error: language selector not found");
		...
		addEvents() {
			document.getElementById("BLICK_plus")?.addEventListener("click", () => {
				...
				(<HTMLInputElement>document.getElementById("BLICK_fontsize")).value = String(this.setting.data.fontsize);
			...
			document.getElementById("BLICK_fontsize")?.addEventListener("exit", (ev) => {
				let num = parseInt((<HTMLInputElement>ev.target)?.value ?? this.setting.default.fontsize);
				if (num > 6)
					this.setting.set("fontsize", num);
				else
					(<HTMLInputElement>ev.target).value = String(this.setting.data.fontsize);
				this.dom.setFontsize(true);
			});
			...
			document.getElementById("BLICK_sync_download")?.addEventListener("click", (ev) => {
				...
				Promise.all([
					...
					Promise.all([
						...
						if (loginHello) loginHello.innerHTML = "Download successful!";
			...
			document.getElementById("BLICK_set_importFile")?.addEventListener("change", (event) => {
				...
				const file = (<HTMLInputElement>event.target).files?.item(0) ?? null;
				if (file === null) return;
				const reader = new FileReader();
				reader.onload = (e) => {
					try {
						const jsonString = e.target?.result as string || null;
	...
	class ImageManager {
		...
		private imgCol!: HTMLElement | null;
		private loading!: HTMLElement | null;
		private allIcons: string[];
		private shownIcons: string[];
		private curActTab: number;
		private spliterDraggin: boolean;
		// private data: { label: string; arr: string[] }[];
		private itemCount: number;
		private itemsPerLoad: number;
		private pickerDiv!: HTMLDivElement;
		private editConf;
		private clickCallback: (icon: string) => void;
		constructor(container: HTMLElement, conf: EditorConfig, clickCallback: (icon: string) => void) {
		...
		//
		private insertHTML() {
			this.pickerDiv = document.createElement("div") as HTMLDivElement;
		...
		private stripIconName(name: string): string {
		...
		private setupEventListeners() {
			...
			document.getElementById("BLICK_iP_seachInp")?.addEventListener("keydown", (ev) => { if (ev.key === 'Enter') ev.preventDefault(); });
			//
			...
			document.getElementById("BLICK_iP_ImgColumn")?.addEventListener('click', (ev) => {
				let el = (ev.target as HTMLElement).closest(".image-container")
				if (el === null) return;
			...
			if (this.loading !== null) observer.observe(this.loading);
		}
		//
		private onMouseDown(event: MouseEvent) {
			const target = event.target as HTMLElement;
			if (target === null) return;
			const clst = target.closest('.image-container') as HTMLElement;
			//
		...
		private onMouseMove(event: MouseEvent) {
			...
			document.querySelectorAll<HTMLElement>('.image-container').forEach((item) => {
		...
		drag(event: DragEvent) {
			if (this.selectedItems.length === 0 && event.currentTarget !== null) {
				this.selectedItems.push(event.currentTarget as HTMLElement);
			}
			event.dataTransfer?.setData('text/plain', JSON.stringify(this.selectedItems.map(item => item.title)));
		}
		//
		enableTabDrop() {
			const BLICK_iP_tabs = document.querySelectorAll<HTMLElement>('#BLICK_iP_tabs div');
			BLICK_iP_tabs.forEach(tab => {
				...
				tab.addEventListener('drop', (event: DragEvent) => {
					...
					if ((event.target as HTMLElement)?.dataset.func == "+") return
					if ((event.target as HTMLElement)?.dataset.role == "delete" || tabId === this.curActTab) { //remove entries
						if (confirm("Remove icons from list?")) {
							let ind = 0;
							titles.forEach((title: string) => {
								ind = this.editConf.data.iconList[tabId].arr.indexOf(title);
								if (ind > -1) this.editConf.data.iconList[tabId].arr.splice(ind, 1);
							});
							this.clearItems();
						}
					} else { //add entries
		...
		//
		searchItems() {
			const search = (<HTMLInputElement>document.getElementById("BLICK_iP_seachInp"))?.value ?? "";
			//
		...
		loadTabs() {
			...
			this.editConf.data.iconList.forEach((item, index) => {
				...
				if (index === this.curActTab) tab.classList.add("cur");
		...
		onTabClick(ev: MouseEvent) {
			...
			if (target.dataset.func === "+") {
				...
				if (newLabel === "" || newLabel === null) return;
				...
				if (!confirm("Do you want to delete this icon list?")) return;
				const index = parseInt(target.parentElement?.dataset.tabid ?? "0");
		...
		//
		loadItems() {
			if (this.imgCol === null || this.loading === null) return;
	...
	class UndoRedoManager {
		...
		private undoStack: { value: string; selectionStart: number; selectionEnd: number }[] = [];
		private redoStack: { value: string; selectionStart: number; selectionEnd: number }[] = [];
		private applyingState: boolean;
		private inputListener: () => void;
		private saveStateTimer: number;
		private saveStateInterval: number;
		...
		//
		private applyState(state: { value: string; selectionStart: number; selectionEnd: number }): void {
	...
	class ReplyForm {
		...
		private colPickEl!: null | { hue: HTMLInputElement, sat: HTMLInputElement, lum: HTMLInputElement, prevCol: HTMLDivElement, bgCol: HTMLDivElement, prevText: HTMLInputElement };
		private iconPicker: ImageManager | null;
		public editConf: EditorConfig;
		private buttonBar: HTMLElement | null;
		private extendBar: HTMLElement | null;
		private undoManager: UndoRedoManager | null;
		private genPrevTimer: number;
		private autoGeneratePrev: boolean;
		private saveTimer: number;
		private saveTimeStamp: HTMLDivElement | null;
		//
		constructor() {
			this.bar = null;
			this.textarea = null;
		...
		insertBar() {
			...
			if (this.textarea === null) { this.bar = null; return }
			//
			...
			autoSaveCB.addEventListener("change", (ev) => {
				let state: boolean = (<HTMLInputElement>ev.target)?.checked ?? true;
			...
			this.addButton("color", 10, () => {
				...
				if (this.iconPicker) this.iconPicker.hide();
			});
			this.addButton("icon", 9, () => {
				if (this.extendBar === null) return;
				if (this.iconPicker === null) {
					this.iconPicker = new ImageManager(this.extendBar, this.editConf, (icon: string) => {
		...
		private addPreviewBut() {
			...
			cbPrev.addEventListener("change", (ev) => {
				this.autoGeneratePrev = (<HTMLInputElement>ev.target)?.checked ?? false;
		...
		private insertColorPicker() {
			...
			//
			this.colPickEl = {
				hue: document.getElementById('BLICK_cp_hue') as HTMLInputElement,
				sat: document.getElementById('BLICK_cp_saturation') as HTMLInputElement,
				lum: document.getElementById('BLICK_cp_lightness') as HTMLInputElement,
				bgCol: document.getElementById('BLICK_cp_bgDisplay') as HTMLDivElement,
				prevCol: document.getElementById('BLICK_cp_colorDisplay') as HTMLDivElement,
				prevText: document.getElementById('BLICK_cp_colTex') as HTMLInputElement
			};
			//
		...
		private updatePickerColer(textupdate: boolean = false) {
			if (this.colPickEl == null) return;
		...
		private insertColorChoice() {
			...
			cp.addEventListener("contextmenu", (ev) => {
				if (!(ev.target instanceof HTMLAnchorElement) || ev.target.className !== "customCol" || this.colPickEl === null) return;
				ev.preventDefault();
				ev.stopPropagation();
				let id: number = parseInt(ev.target.getAttribute("customId") ?? "0");
		...
		private addButton(tag: string, bgpos: number, callback: null | (() => void) = null) {
		...
		//
		private styleSelection(tag: string) {
			if (this.textarea === null) return;
		...
		private debsaveText() {
			if (!this.editConf.data.autoSave) {
				if (this.saveTimeStamp !== null) this.saveTimeStamp.innerText = "";
				this.editConf.set("saveText", "");
				return;
			}
		...
		private saveText(ev: Event | null) {
			ev?.preventDefault();
		...
		private debGeneratePrev() {
			if (!this.autoGeneratePrev) return;
			clearTimeout(this.genPrevTimer);
			this.genPrevTimer = setTimeout(() => {
				this.generatePreview(null);
			}, 500);
		}
		private generatePreview(ev: Event | null) {
			ev?.preventDefault();
			//
			const postForm = document.forms.namedItem("postform");
			...
			//
			const boardValue = (postForm.elements.namedItem("board") as HTMLInputElement)?.value ?? "";
			const replyThreadValue = (postForm.elements.namedItem("replythread") as HTMLInputElement)?.value ?? "";
			const messageValue = (postForm.elements.namedItem("message") as HTMLTextAreaElement)?.value ?? "";
			//
	...
	interface watchdataform {
		...
		default: threaddata,
		numLinkMode: number, //0:new, 1:lastread, 2:none; link to <> post when clicking the number-link
	};
	class WatchData extends Storage<watchdataform> {
		constructor() {
			let def: watchdataform = {
				numLinkMode: 0,
				threads: new Map(),
				default:
				{
	...
	class WatchBar {
		...
		public wdata: WatchData;
		private curThread: threaddata | null = null;
		private saveTimer: number = 0;
		private saveInterval: number = 500;
		private setDiag!: HTMLElement;
		private thList = ["draw", "meep", "quest", "questdis", "tg", "questarch"];
		...
		private insertHTML() {
			...
			wbar.addEventListener("click", (ev) => {
				...
				if (ev.target.dataset.role == "remove") {
					let row = ev.target.closest("div.BLICK_wrow") as HTMLDivElement;
					if (row === null || row.dataset.id === undefined) return;
					if (!confirm(`Remove entry for ${this.wdata.data.threads.get(row.dataset.id)?.label}?`)) return;
		...
		private copyToSite() {
			...
			this.wdata.data.threads.forEach((thread, id) => {
				fetchPromises.push(fetch(`https://questden.org/kusaba/threadwatch.php?do=addthread&board=${thread.section}&threadid=${id}`).then((ret) => {
					if (ret.ok) console.log(`added ${thread?.label}, ${thread?.section}/${id} to your watchlist.`);
				}));
			});
		...
		private insertSettingHTML() {
			...
			this.setDiag.addEventListener("click", function (ev) {
				if (ev.target == this) this.style.display = "none"; //this in function() refers to "this.setDiag"
			});
			document.getElementById("BLICK_wbar_button-row")?.addEventListener("click", (ev) => {
				let target = ev.target as HTMLElement;
				if (target.dataset.role == null || this.setDiag.dataset.threadid === undefined) return;
		...
		private showSettingDiag(id: string, def = false) {
			if (this.setDiag === null || document.getElementById("BLICK_wbar_formSubtitle") == null ||
				(id !== "" && !this.wdata.data.threads.has(id))) return;
			...
			if (id === "") {
				...
				(<HTMLSpanElement>document.querySelector("#BLICK_wbar_threadSec .BLICK_wbar_secTitl")).innerText = "Thread settings";
			}
			//
			(<HTMLInputElement>document.getElementById("BLICK_wbar_highlightNew")).checked = thread?.highImgOnly ?? true;
			(<HTMLInputElement>document.getElementById("BLICK_wbar_highlightNames")).value = thread?.highNames.join(",") ?? "";
			(<HTMLInputElement>document.getElementById("BLICK_wbar_highlightIDs")).value = thread?.highIDs.join(",") ?? "";
			(<HTMLInputElement>document.getElementById("BLICK_wbar_ignoreNames")).innerText = thread?.ignoreNames.join(",") ?? "";
			(<HTMLInputElement>document.getElementById("BLICK_wbar_ignoreIDs")).innerText = thread?.ignoreIDs.join(",") ?? "";
			this.setDiag.style.display = "flex";
			this.setDiag.dataset.threadid = id;
		}
		private saveSettingDiag(id: string, close = false) {
			if (this.setDiag === null || document.getElementById("BLICK_wbar_formSubtitle") == null ||
				(id !== "" && !this.wdata.data.threads.has(id))) return;
			const thread = (id !== "") ? this.wdata.data.threads.get(id)! : this.wdata.data.default;
			//
			let sanit = (str: string) => { return str.split(",").map(name => name.trim()).filter(name => name !== "") };
			thread.highImgOnly = (<HTMLInputElement>document.getElementById("BLICK_wbar_highlightNew")).checked ?? true;
			thread.highNames = sanit((<HTMLInputElement>document.getElementById("BLICK_wbar_highlightNames")).value);
			thread.highIDs = sanit((<HTMLInputElement>document.getElementById("BLICK_wbar_highlightIDs")).value);
			thread.ignoreNames = sanit((<HTMLInputElement>document.getElementById("BLICK_wbar_ignoreNames")).value);
			thread.ignoreIDs = sanit((<HTMLInputElement>document.getElementById("BLICK_wbar_ignoreIDs")).value);
			if (id === "") {
				this.sets.set("autoUpdWbar", (<HTMLInputElement>document.getElementById("BLICK_wbar_autoUpdate")).checked ?? true);
				this.wdata.set("numLinkMode", (<HTMLSelectElement>document.getElementById("BLICK_wbar_linkTo")).selectedIndex ?? 0);
			}
			//
			this.debSave();
			if (close) this.setDiag.style.display = "none";
		}
		private debSave() { //debounce saving, so only once in 500ms is saved at max.
			clearTimeout(this.saveTimer);
			this.saveTimer = setTimeout(() => {
				this.wdata.save();
				this.sets.save();
		...
		private scrollObserver(entries: IntersectionObserverEntry[], observer: IntersectionObserver): void {
			let topElement = document.elementFromPoint(300, 0); //get top element at 300px to left
			if (!topElement || topElement.tagName === "HTML") topElement = document.elementFromPoint(300, 20); //if gap, check 20px below
		...
		private addThread(id: string, board: string) {
			if (this.wdata.data.threads.has(id)) return;
		...
		private insertObserver() {
			if (this.sets.data.autoUpdWbar) this.updateEntries(); //autoupdate on pageload
			document.querySelectorAll<HTMLImageElement>("img.watchthread").forEach((el) => {
				const clickstr = el.parentElement?.getAttribute("onclick");
				const threadData = clickstr?.match(/addtowatchedthreads\('(.+?)','(.+?)'\)/) ?? null;
				if (threadData === null || threadData.length < 3) return;
				...
				el.addEventListener("click", (ev) => {
					...
					if (target.dataset.targetid === undefined || target.dataset.targetboard === undefined) return;
			...
			let id = locMatch[2];
			if (!this.wdata.data.threads.has(id)) return; //log data only for watched websites.
			...
			if (currentReadPost) currentReadPost.scrollIntoView(); //executed before Quest Reader, so should not collide.
			//
			const replyElements = document.querySelectorAll('td.reply');
			const lastmatch = replyElements[replyElements.length - 1].id.match(/\d+/)
			if (lastmatch !== null && this.curThread !== null) this.curThread.lastReadId = lastmatch[0];
			this.wdata.save();
			//
		...
		private updateEntries() {
			...
			this.wdata.data.threads.forEach((thread, id) => {
				fetchPromise.push(
					fetch(`https://questden.org/kusaba/quest/res/${id}.html`)
						.then(response => response.text())
						.then(htmlText => {
							...
							//
							//parse website
							let lastReadElement: null | HTMLTableRowElement = null;
							let newCount = 0;
							const replyElements = doc.querySelectorAll<HTMLTableRowElement>(`td.reply`);
							...
							thread.label = (<HTMLSpanElement | null>doc.querySelector("div.postwidth span.filetitle"))?.innerText.trim() ?? thread.label;
							thread.author = (<HTMLSpanElement | null>doc.querySelector("div.postwidth span.postername"))?.innerText.trim() ?? thread.author;
							this.wdata.save();
							//
							//update list
							let entrNewCnt = document.querySelector<HTMLAnchorElement>(`div.BLICK_wrow[data-id='${id}'] div.BLICK_wnew a`);
							if (entrNewCnt !== null) {
								let add = "";
								if (thread.newEntrCnt < 50 && thread.totalEntrCnt > 50) add = "+50";
								else if (thread.newEntrCnt < 100 && thread.totalEntrCnt > 100) add = "+100";
								let jump = "";
							...
							let entrLabel = document.querySelector<HTMLAnchorElement>(`div.BLICK_wrow[data-id='${id}'] div.BLICK_wtitle a`);
							if (entrLabel !== null) entrLabel.innerText = thread.label;
							let entrAuthor = document.querySelector<HTMLAnchorElement>(`div.BLICK_wrow[data-id='${id}'] div.BLICK_wauthor`);
							if (entrAuthor !== null) entrAuthor.innerText = thread.author;
						})
				);
			});
		...
		private insertEntries() {
			...
			const groupedThreads: Map<string, string[]> = new Map(this.thList.map(section => [section, []]));
			...
			groupedThreads.forEach((threads, section) => {
				...
				threads.forEach(id => {
					...
					let add = "";
					if (el.newEntrCnt < 50 && el.totalEntrCnt > 50) add = "+50";
					else if (el.newEntrCnt < 100 && el.totalEntrCnt > 100) add = "+100";
					let jump = "";
					if (this.wdata.data.numLinkMode == 0 && el.lastReadId !== "") jump = "#" + el.lastReadId;
					if (this.wdata.data.numLinkMode == 1 && el.currentReadId !== "") jump = "#" + el.currentReadId;
					//
	...
	class FileExport {
		...
		private filter: epubFilter;
		private title = "";
		private author = "";
		private entryList: HTMLElement[] = [];
		private statDispEl!: HTMLElement;
		constructor() {
			this.insertHTML();
			this.insertStyle();
			this.filter = {
		...
		showExportForm(mode = "epub") {
			...
			(<HTMLInputElement>document.getElementById("BLICK_epub_excludeAuthors")).value = this.filter.exclude;
			(<HTMLSelectElement>document.getElementById("BLICK_epub_styleSelect")).selectedIndex = this.filter.style;
			(<HTMLSelectElement>document.getElementById("BLICK_epub_imageQuality")).selectedIndex = this.filter.imgQuality;
			...
			(<HTMLSelectElement>document.getElementById("BLICK_epub_author")).value = this.author;
			this.filterEntries(document);
		}
		hideEPUBForm() { this.form.style.display = "none"; }
		private transferSettings() {
			this.filter.onlyCheck = (<HTMLInputElement>document.getElementById("BLICK_epub_includeChecked")).checked;
			this.filter.onlyImg = (<HTMLInputElement>document.getElementById("BLICK_epub_includeImages")).checked;
			this.filter.include = (<HTMLInputElement>document.getElementById("BLICK_epub_includeAuthors")).value;
		...
		private filterEntries(cont: HTMLElement | Document) {
			this.entryList = [];
			let imgCnt = 0;
		...
		private detectPosterName() {
			if (this.form === null) return;
			const authNam = new Set(), authID = new Set();
			document.querySelectorAll("img.thumb").forEach(el => {
				const par = el.closest("div.postwidth") as HTMLDivElement | null;
				if (par === null) return;
				authNam.add((<HTMLSpanElement | null>par.querySelector("span.postername"))?.innerText);
				authID.add((<HTMLSpanElement | null>par.querySelector("span.uid"))?.innerText.substring(4));
			});
			const resStr = [...authNam, ...authID].join(", ");
			(<HTMLInputElement>document.getElementById("BLICK_epub_includeAuthors")).value = resStr;
		}
		private selectEntries() {
			this.filterEntries(document);
			document.querySelectorAll<HTMLInputElement>("input[type='checkbox'][name='post[]']").forEach(el => {
				el.checked = false;
				el.parentElement!.classList.remove("BLICK_epub_selected");
			});
			this.entryList.forEach(el => {
				let cb = el.querySelector("input[type='checkbox'][name='post[]']") as HTMLInputElement;
				cb.checked = true;
		...
		private exportEpub() {
			...
			const cont = this.entryList.map((el, ind, arr) => {
				...
				let cont = el.parentElement;
				//
				const author = (<HTMLSpanElement | null>cont?.querySelector("span.postername"))?.innerText ?? "Bob";
				const title = (<HTMLSpanElement | null>cont?.querySelector("span.filetitle"))?.innerText ?? "";
	...
	class DOMManip {
		private styEl!: HTMLStyleElement;
		private hoverImgCont!: HTMLDivElement;
		private wartImg!: HTMLImageElement;
		private fullImg!: HTMLImageElement;
		private styles!: { fontsize: string, paragraphMargin: string, invertCol: string, replyForm: string };
		private setting: Settings;
		public repform: ReplyForm;
		public watchbar: WatchBar | null;
		//
		...
		setReplyForm(update = false) {
			if (!this.setting.data.replyForm) {
				...
				document.querySelectorAll(".postarea table tr td:nth-of-type(2)").forEach(el => {
					const prevEl: HTMLInputElement | null = el.querySelector("input[type='text']") ?? null;
				...
				if (pwbox?.nextSibling === null) pwbox?.insertAdjacentHTML("afterend", "<span></span>"); //questReader workaround for .nextSibling.remove()
		...
		setWatchbar() {
			if (this.setting.data.manageWatchlist) {
				if (this.watchbar === null) this.watchbar = new WatchBar(this.setting);
			} else {
		...
		setHoverImg() {
			if (this.setting.data.imageHover) {
				document.querySelectorAll('img.thumb:not([BLICK_Hover])').forEach(img => {
					img.addEventListener('mouseover', this.EvImgHoverIn as EventListener);
					img.addEventListener('mouseleave', this.EvImgHoverOut as EventListener);
				});
			} else {
				document.querySelectorAll("img.thumb[BLICK_Hover]").forEach(img => {
					img.removeEventListener("mouseover", this.EvImgHoverIn as EventListener);
					img.removeEventListener("mouseleave", this.EvImgHoverOut as EventListener);
					img.removeAttribute("BLICK_Hover");
				});
			}
		}
		private EvImgHoverIn(ev: MouseEvent): void {
			if (!(ev.target instanceof HTMLImageElement)) return;
			//
			const linkEl = ev.target.parentNode?.parentNode as HTMLAnchorElement;
		...
		private EvImgHoverOut(ev: MouseEvent): void {
			if (!this.hoverImgCont.contains(ev.relatedTarget as Node)) {
	...
	function main() {
		...
		delete (Array.prototype as any).toJSON; //remove prototype.js prototype changes to stringify()
